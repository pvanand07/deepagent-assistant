"""Environment-driven Bubblewrap configuration."""
...
def resolve_data_dir() -> Path:
    """Config/DB/logs root: ``DEEPAGENT_DATA_DIR``, else repository ``data/``."""
...
def sandbox_dns_nameservers() -> tuple[str, ...]:
    ...
    + Google. Set to empty / ``host`` to use the host resolver.
    """
...
def sandbox_recreate_fingerprint() -> tuple[Any, ...]:
    """Effective sandbox settings; changes may recreate the sandbox backend."""
    return (
        default_network(),
        sandbox_memory_mib(),
        sandbox_cpus(),
