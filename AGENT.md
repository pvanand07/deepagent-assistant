...
- Guest image includes `officecli` (+ libicu), Chromium (+ fonts-liberation), Node.js/npm, and the globally installed `chrome-devtools-axi` / `chrome-devtools-mcp` tools. Skills: `skills/officecli/` and `skills/chrome-devtools-axi/`.
