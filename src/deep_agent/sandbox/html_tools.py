"""Rendered HTML inspection and screenshot helpers.

Chromium runs on the **host** (app container), not inside Bubblewrap. Nested
user namespaces (Docker + bwrap) make Chromium trap; workspace paths are still
validated under the sandbox workdir. ``officecli`` and other shell tools
continue to use Bubblewrap ``execute``.
"""
...
def _chromium_bin() -> str:
    for name in ("chromium", "chromium-browser", "google-chrome", "chrome"):
        ...
        if found:
            return found
    raise FileNotFoundError("Chromium is not installed on the host PATH")
...
async def inspect_html_file(
    ...
    *,
    ...
    exit_code, stdout, stderr = await _run_host_chromium(args, timeout=30)
    dom = stdout
    lower = dom.lower()
    ...
    browser_errors = [
        line.strip() for line in stderr.splitlines() if _ERROR_RE.search(line)
    ][:50]
    missing_assets = _missing_local_assets(
        manager.workdir, source, report.get("resource_urls") or []
    )
    chromium_ok = exit_code in (0, None)
    report.update(
        {
    ...
    *,
    ...
    if destination.exists() and not overwrite:
        raise FileExistsError(
            f"output already exists: {_guest_path(manager.workdir, destination)}"
        )
...
async def _run_host_chromium(
    args: list[str], *, timeout: float = 30
) -> tuple[int, str, str]:
...
def _file_uri(workdir: Path, source: Path) -> str:
    """Host file URI for Chromium (workdir only used for containment checks)."""
    del workdir
