...
The DeepAgent guest image includes the CLI. Invoke it directly with
`chrome-devtools-axi <command>`. If the binary is unavailable, use the portable
fallback `npx -y chrome-devtools-axi <command>`.
If output shows a follow-up command starting with `chrome-devtools-axi`, run it
directly as printed.
