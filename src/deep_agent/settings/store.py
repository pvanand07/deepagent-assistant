...
def _legacy_env_values() -> dict[str, str]:
    """Read legacy repo ``.env`` without overriding process environment."""
...
def _migrate_legacy_settings() -> tuple[dict[str, Any], dict[str, Any]]:
    """Best-effort one-time import from .env into JSON settings/secrets."""
    legacy = _legacy_env_values()
    settings = default_settings()
    secrets = default_secrets()
    platform = settings["platforms"][0]
    model = legacy.get("OPENROUTER_MODEL") or os.environ.get("OPENROUTER_MODEL")
    if model:
        settings["default_model"] = model
        platform["models"] = [{"id": model, "enabled": True, "temperature": DEFAULT_MODEL_TEMPERATURE}]
    for key, setting_key in (("OPENROUTER_TEMPERATURE", "temperature"),):
        raw = legacy.get(key) or os.environ.get(key)
        if raw:
            try:
                settings[setting_key] = float(raw)
            except ValueError:
                pass
    api_key = legacy.get("OPENROUTER_API_KEY") or os.environ.get("OPENROUTER_API_KEY")
    if api_key:
        secrets["platforms"]["openrouter"] = {"api_keys": [api_key]}
...
def load_settings(*, force: bool = False) -> dict[str, Any]:
    ...
    if raw is None:
        settings, secrets = _migrate_legacy_settings()
        # Persist the import so later launches are fully JSON-backed.
        _write_json(path, settings)
        _write_json(secrets_file, secrets, secret=True)
        _cache = settings
    ...
    # A migrated or manually provisioned provider is enough to skip Setup.
    active = active_platform(settings)
    if not settings["setup_complete"] and (
        active.get("kind") == KIND_OLLAMA
        or platform_api_key(str(active["id"]), secrets)
