...
1. **Persistence / schema change** — task modifies `src/deep_agent/persistence/` overlapping the topic's code-paths.
2. **API change** — task modifies `src/deep_agent/api/`, FastAPI route handlers, or request/response models overlapping code-paths.
3. **Desktop shell change** — task modifies `src-tauri/`, `frontend/`, or `sidecar/` overlapping code-paths.
4. **Agent / sandbox change** — task modifies `src/deep_agent/agent_factory.py`, `src/deep_agent/sandbox/`, or `src/deep_agent/integrations/` overlapping code-paths.
5. **Refactor flag** — task marked `[REFACTOR]` touches code-paths.
6. **Manual audit** — `/wiki-lint --fix` or manual re-verify.
...
- Session notes → wiki/archive/ or change-logs/
- Unimplemented plans → wiki/archive/ or change-logs/
- Things code already makes obvious → nothing (use LSP, code graph tools)
- Dated files (yyyymmdd-*.md) → never in wiki
- Existing narrative docs under `docs/` → keep there; wiki captures decisions, not full guides
